...
func TestOrderService_CancelOrder(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			orderID:       111,
			payStockOrder: &model.Order{},
			foundOrder:    &model.Order{Status: model.OrderStatusCancelled},
		...
		},
		{
			...
			payStockOrder: &model.Order{Status: model.OrderStatusCancelled},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			stockService.CancelStocksMock.Expect(ctx, *tt.payStockOrder).Return(tt.payStocksErr)
			orderRepository.SaveMock.Expect(ctx, tt.payStockOrder).Return(nil, tt.orderSaveErr)

			err := orderService.CancelOrder(ctx, tt.orderID)
