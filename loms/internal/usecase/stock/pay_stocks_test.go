...
func TestPayStocks(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		},
		{
			...
			saveReservedStock: &model.ReservedStock{Sku: 111, OrderID: 1, Count: 2, Status: model.ReservedStockStatusPaid},
			sku:               111,
			stock:             &model.Stock{Sku: 111, TotalCount: 2},
			wantErr:           nil,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := stockService.PayStocks(ctx, tt.order)
